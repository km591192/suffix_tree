...
namespace Suftree
{
    public class SuffixTree
    {
        ...
        public class Edge
        {
            ...
            {
                return string.Concat(_tree.Word.Substring(StartIndex, ((EndIndex) ?? _tree.CurrentSuffixEndIndex - 1) - StartIndex + 1), "(",
                    StartIndex, ",", EndIndex.HasValue ? EndIndex.ToString() : "#", ")");
            }

            public string StringWithoutCanonizationChar
            {
                get { return _tree.Word.Substring(StartIndex, ((EndIndex) ?? (_tree.CurrentSuffixEndIndex - 1) - (_tree.CanonizationChar.HasValue ? 1 : 0)) - StartIndex + 1); }
            }

            public string String
            {
                get { return _tree.Word.Substring(StartIndex, ((EndIndex) ?? (_tree.CurrentSuffixEndIndex - 1)) - StartIndex + 1); }
            }

            public void RenderTree(TextWriter writer, string prefix, int maxEdgeLength)
            {
                var strEdge = _tree.Word.Substring(StartIndex, ((EndIndex) ?? _tree.CurrentSuffixEndIndex - 1) - StartIndex + 1);
